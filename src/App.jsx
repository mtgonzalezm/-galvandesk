...
const PASSWORDS = {
  jefatura:       "jefatura2024",
  administracion: "admin2024",
};
...
// ─── App principal ────────────────────────────────────────────────────────────
// ═══════════════════════════════════════════════════════════════════════════
// LOGIN SCREEN con contraseña
// ═══════════════════════════════════════════════════════════════════════════
function LoginScreen({ onLogin, C }) {
  const [loginPerfil, setLoginPerfil] = useState(null);
  const [pass, setPass]               = useState("");
  const [error, setError]             = useState(false);

  function intentarLogin(perfil) {
    if (perfil.id === "profesor") { onLogin(perfil); return; }
    setLoginPerfil(perfil);
    setPass(""); setError(false);
  }

  function confirmarPass() {
    const key = loginPerfil.id === "jefatura" ? "jefatura" : "administracion";
    if (pass === PASSWORDS[key]) { onLogin(loginPerfil); }
    ...
    <div style={{ minHeight:"100vh", background:`linear-gradient(135deg,${C.dark},${C.blue})`, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"system-ui,sans-serif", padding:20 }}>
      ...
      <div style={{ background:C.white, borderRadius:20, padding:"40px 36px", maxWidth:420, width:"100%", textAlign:"center", boxShadow:"0 20px 60px rgba(0,0,0,0.3)" }}>
        ...
        {!loginPerfil ? (
          <>
            {[
              { id:"profesor",  label:"👨‍🏫 Profesor",      sinPass:true  },
              { id:"jefatura",  label:"📊 Jefatura / Dirección", sinPass:false },
              { id:"admin",     label:"⚙️ Administración",  sinPass:false },
            ].map(p => (
              <button key={p.id} onClick={() => intentarLogin(p)}
                ...
                {p.label} {!p.sinPass && <span style={{ fontSize:12, opacity:0.6 }}>🔒</span>}
              </button>
            ))}
          </>
        ) : (
          <div>
            ...
            <div style={{ fontWeight:700, fontSize:16, color:C.dark, marginBottom:4 }}>{loginPerfil.label}</div>
            <p style={{ color:C.gray, fontSize:13, marginBottom:20 }}>Introduce la contraseña de acceso</p>
            <input
              type="password"
              value={pass}
              onChange={e => { setPass(e.target.value); setError(false); }}
              onKeyDown={e => { if(e.key==="Enter") confirmarPass(); }}
              placeholder="Contraseña..."
              autoFocus
              style={{ width:"100%", padding:"12px 16px", borderRadius:10, border:`2px solid ${error?C.salmon:C.teal}`, fontSize:16, marginBottom:8, textAlign:"center", outline:"none" }}
            />
            {error && <div style={{ color:C.salmon, fontSize:13, marginBottom:10, fontWeight:600 }}>❌ Contraseña incorrecta</div>}
            <div style={{ display:"flex", gap:10, marginTop:8 }}>
              <button onClick={() => setLoginPerfil(null)}
                style={{ flex:1, padding:"12px", borderRadius:10, border:`1px solid #ccc`, background:"#f5f5f5", cursor:"pointer", fontWeight:600, fontSize:14 }}>
                ← Volver
              </button>
              <button onClick={confirmarPass}
                style={{ flex:2, padding:"12px", borderRadius:10, border:"none", background:C.teal, color:"#fff", cursor:"pointer", fontWeight:700, fontSize:14 }}>
                Entrar
              </button>
            </div>
          </div>
    ...
    <div>
      ...
      {esFinde ? (
        ...
        <div>
          ...
          {guardiasDia.map((g, i) => {
            const esRecreoo = g.hora === "Recreo";
            const color = esRecreoo ? C.blue : C.teal;
            return (
              <div key={i} style={{ background:C.white, borderRadius:12, padding:18, marginBottom:12, boxShadow:"0 2px 10px rgba(0,0,0,0.06)", borderLeft:`5px solid ${color}` }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <div>
                    ...
                    <div style={{ fontSize:15, color:color, fontWeight:600, marginTop:4 }}>📍 {g.zona}</div>
                  </div>
                  <div style={{ background:esRecreoo?"#EEF5F8":"#E8F5F3", borderRadius:10, padding:"8px 16px", fontSize:13, fontWeight:700, color }}>
                    {esRecreoo ? "RECREO" : "GUARDIA"}
                  </div>
                </div>
              </div>
            );
          ...
          <div style={{ background:"#FFF8E8", borderRadius:10, padding:14, marginTop:8, fontSize:13, color:C.dark, border:`1px solid #fbbf24` }}>
            ⚠️ Si no puedes asistir a alguna guardia, notifícalo cuanto antes en la pestaña <strong>Notificar Ausencia</strong>.
          </div>
        </div>
      )}
    </div>
    ...
    <div>
      ...
      {enviado && (
        <div style={{ background:"#E8F5F3", border:`2px solid ${C.teal}`, borderRadius:12, padding:16, marginBottom:16, fontWeight:700, color:C.teal, fontSize:15 }}>
          ✅ Ausencia notificada correctamente. Jefatura ha sido informada.
        </div>
      )}
...
// ═══════════════════════════════════════════════════════════════════════════
// CUADRANTE DE GUARDIAS (Jefatura) — constructor
// ═══════════════════════════════════════════════════════════════════════════
function CuadranteGuardias({ profesores, cuadrante, setCuadrante, quinceInicio, setQInicio, quinceProfesor, setQProf, C, inpStyle, selStyle, labelStyle }) {
  function setAsignacion(dia, hora, profesor, zonaId) {
    ...
    setCuadrante(prev => {
      const next = { ...prev };
      if (zonaId === "") delete next[key];
      else next[key] = zonaId;
      return next;
    });
  }

  function getAsignacion(dia, hora, profesor) {
    return cuadrante[`${dia}|${hora}|${profesor}`] || "";
  }
  ...
  return (
    <div>
      ...
      <div style={{ background:C.white, borderRadius:12, padding:16, boxShadow:"0 2px 10px rgba(0,0,0,0.06)", overflowX:"auto" }}>
        <div style={{ fontWeight:700, color:C.dark, marginBottom:14, fontSize:14 }}>
          Asignación de {profesorSel} — selecciona zona para cada hora
        </div>
        <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12, minWidth:600 }}>
          <thead>
            <tr style={{ background:C.dark }}>
              ...
              {DIAS_SEMANA.map(d => (
                <th key={d} style={{ padding:"8px 12px", color:"#fff", textAlign:"left" }}>{d}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {HORAS_GUARDIA.map((hora, i) => (
              <tr key={hora} style={{ background:i%2===0?"#fff":C.light }}>
                ...
                {DIAS_SEMANA.map(dia => {
                  const val = getAsignacion(dia, hora, profesorSel);
                  const z   = ZONAS_CENTRO.find(z => z.id === val);
                  return (
                    <td key={dia} style={{ padding:"4px 8px" }}>
                      <select
                        value={val}
                        onChange={e => setAsignacion(dia, hora, profesorSel, e.target.value)}
                        ...
                        <optgroup label="── Edificio A">
                          {ZONAS_CENTRO.filter(z=>z.edificio==="A").map(z=><option key={z.id} value={z.id}>{z.label}</option>)}
                        </optgroup>
                        <optgroup label="── Edificio B">
                          {ZONAS_CENTRO.filter(z=>z.edificio==="B").map(z=><option key={z.id} value={z.id}>{z.label}</option>)}
                        </optgroup>
                        <optgroup label="── Edificio C">
                          {ZONAS_CENTRO.filter(z=>z.edificio==="C").map(z=><option key={z.id} value={z.id}>{z.label}</option>)}
                        </optgroup>
                        <optgroup label="── Aula / Recreo">
                          {ZONAS_CENTRO.filter(z=>z.edificio==="-").map(z=><option key={z.id} value={z.id}>{z.label}</option>)}
                        </optgroup>
                      </select>
                    </td>
                  );
        ...
        <div style={{ marginTop:14, fontSize:12, color:C.gray }}>
          💾 Los cambios se guardan automáticamente. Configura profesor por profesor.
        </div>
      </div>
    </div>
  );
...
// ═══════════════════════════════════════════════════════════════════════════
// PARTE DEL DÍA (Jefatura) — tabla cubierto/descubierta
// ═══════════════════════════════════════════════════════════════════════════
function ParteDia({ profesores, cuadrante, ausencias, C, fmt }) {
  const hoy     = new Date();
  const diasES  = ["Domingo","Lunes","Martes","Miércoles","Jueves","Viernes","Sábado"];
  const diaHoy  = diasES[hoy.getDay()];
  const fechaHoy = hoy.toISOString().split("T")[0];

  // Ausencias notificadas para hoy
  const ausHoy = ausencias.filter(a => a.fecha === fechaHoy);
  const profesoresAusentesHoy = new Set(ausHoy.flatMap(a => a.horas.map(h => `${a.profesor}|${h}`)));

  // Construir mapa de todas las zonas asignadas hoy con su estado
  const asignaciones = [];
  HORAS_GUARDIA.forEach(hora => {
    profesores.forEach(prof => {
      const key  = `${diaHoy}|${hora}|${prof}`;
      const zona = cuadrante[key];
      if (!zona) return;
      const z       = ZONAS_CENTRO.find(z => z.id === zona);
      const ausente = profesoresAusentesHoy.has(`${prof}|${hora}`);
      asignaciones.push({ hora, profesor: prof, zona: z?.label || zona, ausente });
    });
  });

  // Agrupar por hora
  const porHora = HORAS_GUARDIA.map(hora => ({
    hora,
    items: asignaciones.filter(a => a.hora === hora),
  })).filter(h => h.items.length > 0);

  const totalCubiertas   = asignaciones.filter(a => !a.ausente).length;
  const totalDescubiertas = asignaciones.filter(a => a.ausente).length;
    ...
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12, marginBottom:16 }}>
        {[
          ...
          { label:"Cubiertas",       value:totalCubiertas,      color:C.teal },
          { label:"Descubiertas",    value:totalDescubiertas,   color:C.salmon },
        ].map(s => (
      ...
      {ausHoy.length > 0 && (
        <div style={{ background:"#FDF0EF", border:`1px solid ${C.salmon}`, borderRadius:10, padding:12, marginBottom:16, fontSize:13 }}>
          ...
          {ausHoy.map(a => (
            <div key={a.id} style={{ marginTop:4, color:C.dark }}>
              · {a.profesor} — {a.horas.join(", ")} — {a.motivo}
              {a.tarea && <span style={{ color:C.gray }}> · Tarea: {a.tarea.slice(0,50)}</span>}
            </div>
          ))}
        </div>
      )}
      ...
      {porHora.length === 0 ? (
        <div style={{ background:C.white, borderRadius:12, padding:30, textAlign:"center", color:C.gray, boxShadow:"0 2px 10px rgba(0,0,0,0.06)" }}>
          No hay guardias configuradas para hoy.<br/>
          <span style={{ fontSize:13 }}>Ve a <strong>Cuadrante Guardias</strong> para asignar zonas.</span>
        </div>
      ) : (
...
// ═══════════════════════════════════════════════════════════════════════════
function GestionAusencias({ ausencias, setAusencias, profesores, cuadrante, C, fmt }) {
  const [filtFecha, setFiltFecha] = useState("");
  const [filtProf,  setFiltProf]  = useState("");

  const ausFiltradas = ausencias.filter(a => {
    if (filtFecha && a.fecha !== filtFecha)           return false;
    if (filtProf  && a.profesor !== filtProf)         return false;
    return true;
  });
  ...
  return (
    <div>
      ...
      <div style={{ background:C.white, borderRadius:12, padding:16, marginBottom:14, boxShadow:"0 2px 8px rgba(0,0,0,0.06)" }}>
        ...
        <button onClick={()=>{setFiltFecha("");setFiltProf("");}}
          style={{ marginTop:10, background:"none", border:"1px solid #d1d5db", borderRadius:8, padding:"6px 14px", cursor:"pointer", fontSize:12, color:C.gray }}>
          Limpiar filtros
        </button>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12, marginBottom:16 }}>
        {[
          { label:"Total notificadas", value:ausencias.length,                                    color:C.dark  },
          { label:"Sin leer",          value:ausencias.filter(a=>!a.leida).length,                color:C.salmon},
          { label:"Con tarea dejada",  value:ausencias.filter(a=>a.tarea&&a.tarea.trim()).length, color:C.teal  },
        ].map(s=>(
          <div key={s.label} style={{ background:C.white, borderRadius:10, padding:14, textAlign:"center", boxShadow:"0 2px 8px rgba(0,0,0,0.06)", borderTop:`4px solid ${s.color}` }}>
      ...
      {ausFiltradas.length === 0 ? (
        <div style={{ background:C.white, borderRadius:12, padding:30, textAlign:"center", color:C.gray, boxShadow:"0 2px 8px rgba(0,0,0,0.06)" }}>
          No hay ausencias notificadas
        </div>
      ) : ausFiltradas.map(a => (
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:8 }}>
            <div>
              ...
              <div style={{ fontSize:13, color:C.gray, marginTop:3 }}>
                📅 {new Date(a.fecha).toLocaleDateString("es-ES")} · Motivo: {a.motivo}
              </div>
              <div style={{ fontSize:13, color:C.dark, marginTop:3 }}>
                ⏰ Horas afectadas: <strong>{a.horas.join(", ")}</strong>
              </div>
              {a.tarea && (
                <div style={{ fontSize:13, color:C.dark, marginTop:6, background:C.light, borderRadius:6, padding:"6px 10px" }}>
                  📝 Tarea dejada: {a.tarea}
                </div>
              )}
            </div>
...
export default function App() {
  ...
  const [cuadrante, setCuadrante]   = useState({});   // { "Lunes|1ª hora|profesorId": zonaId }
  const [ausencias, setAusencias]   = useState([]);   // notificaciones de ausencia
  const [quinceInicio, setQInicio]  = useState("");   // fecha inicio quincena
  const [quinceProfesor, setQProf]  = useState("");   // profesor seleccionado en cuadrante
  ...
  // ── Pantalla de selección de perfil con contraseña ──
  if (!perfil) return (
    <LoginScreen
      onLogin={(p) => { setPerfil(p); setTab(p.id === "jefatura" ? "dashboard" : p.id === "admin" ? "admin_panel" : "partes"); }}
      C={C}
    />
  );
  ...
  const tabs = perfil.id === "profesor"
    ...
    ? [
        ...
        { id: "cuadrante",       label: "📅 Cuadrante Guardias" },
    ...
    : [
        ...
        { id: "admin_zonas",      label: "🏫 Zonas del Centro" },
      ];
      ...
      <div style={{ width: "100%", maxWidth: 1100, margin: "0 auto", padding: "20px 24px" }}>
        ...
        {tab === "notif_ausencia" && (
          <NotificarAusencia
            ...
            ausProfesor={ausProfesor}
            setAusProfesor={setAusProfesor}
            ausMotivo={ausMotivo}
            setAusMotivo={setAusMotivo}
            ausFecha={ausFecha}
            setAusFecha={setAusFecha}
            ausHoras={ausHoras}
            setAusHoras={setAusHoras}
            ausTarea={ausTarea}
            setAusTarea={setAusTarea}
            fProfesor={fProfesor}
            C={C}
            inpStyle={inpStyle}
            selStyle={selStyle}
        ...
        {tab === "cuadrante" && (
          <CuadranteGuardias
            profesores={profesores}
            cuadrante={cuadrante}
            setCuadrante={setCuadrante}
            quinceInicio={quinceInicio}
            setQInicio={setQInicio}
            quinceProfesor={quinceProfesor}
            setQProf={setQProf}
            C={C}
            inpStyle={inpStyle}
            selStyle={selStyle}
        ...
        {/* ── Zonas del Centro (Admin) ── */}
        {tab === "admin_zonas" && (
          <div>
            <h2 style={{ color:C.dark, marginTop:0 }}>🏫 Zonas del Centro</h2>
            <div style={{ background:"#EEF5F8", border:`1px solid ${C.blue}`, borderRadius:10, padding:14, marginBottom:16, fontSize:13, color:C.blue }}>
              💡 Estas son las zonas predefinidas del IES Enrique Tierno Galván. Se usan en el cuadrante de guardias y en el parte del día.
            </div>
            {["pasillo","bano","especial","aula","recreo"].map(tipo => {
              const zonas = ZONAS_CENTRO.filter(z => z.tipo === tipo);
              const titulos = { pasillo:"🏃 Pasillos", bano:"🚻 Baños", especial:"⭐ Espacios especiales", aula:"📚 Aulas (sustitución)", recreo:"☀️ Zonas de recreo" };
              const colores = { pasillo:C.dark, bano:C.blue, especial:C.teal, aula:C.salmon, recreo:"#d97706" };
              return (
                <div key={tipo} style={{ background:"#fff", borderRadius:10, marginBottom:12, overflow:"hidden", boxShadow:"0 2px 8px rgba(0,0,0,0.06)" }}>
                  <div style={{ background:colores[tipo], color:"#fff", padding:"10px 16px", fontWeight:700, fontSize:14 }}>{titulos[tipo]}</div>
                  {zonas.map(z => (
                    <div key={z.id} style={{ padding:"10px 16px", borderBottom:`1px solid ${C.cream}`, fontSize:13, color:C.dark }}>
                      📍 {z.label}
                    </div>
                  ))}
                </div>
